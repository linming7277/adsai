...
CREATE INDEX idx_url_visit_results_account_id ON url_visit_results(account_id);
CREATE INDEX idx_url_visit_results_task_id ON url_visit_results(task_id);
CREATE INDEX idx_url_visit_results_batch_id ON url_visit_results(batch_id);
CREATE INDEX idx_url_visit_results_result_type ON url_visit_results(result_type);
CREATE INDEX idx_url_visit_results_domain ON url_visit_results(domain);
CREATE INDEX idx_url_visit_results_created_at ON url_visit_results(created_at DESC);

-- 复合索引（常见查询组合）
CREATE INDEX idx_url_visit_results_user_batch ON url_visit_results(user_id, batch_id);
CREATE INDEX idx_url_visit_results_user_created ON url_visit_results(user_id, created_at DESC);
CREATE INDEX idx_url_visit_results_source_url_hash ON url_visit_results(source_url_hash);
CREATE INDEX idx_url_visit_results_user_url_hash ON url_visit_results(user_id, source_url_hash);
...
CREATE TRIGGER url_visit_results_updated_at
...
-- 用户可以插入自己的结果
CREATE POLICY "Users can insert their own visit results"
  ON url_visit_results
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- 用户可以更新自己的结果
CREATE POLICY "Users can update their own visit results"
  ON url_visit_results
  FOR UPDATE
  USING (auth.uid() = user_id);

-- 用户可以删除自己的结果
CREATE POLICY "Users can delete their own visit results"
  ON url_visit_results
  FOR DELETE
  USING (auth.uid() = user_id);
