...
UI库: Radix UI (headless) + Tailwind CSS v3
状态: Zustand + TanStack Query + SWR
动画: Framer Motion
图表: Recharts
```
...
### 升级后技术栈（方案一：现代化全栈方案）
```
框架: Next.js 15 (Turbopack + React Server Components)
UI库: shadcn/ui (基于Radix) + Tailwind CSS v4
状态: TanStack Query v5 + Zustand v5（精简）
动画: Motion (Framer Motion轻量版)
图表: Tremor 或 Visx
...
<Dialog.Root>
  <Dialog.Trigger className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
    Open
  </Dialog.Trigger>
  <Dialog.Portal>
    <Dialog.Overlay className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm" />
    <Dialog.Content className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200">
      {/* 内容 */}
    </Dialog.Content>
  </Dialog.Portal>
</Dialog.Root>
...
**性能对比**（10,000数据点）：
- Recharts: ~800ms 渲染时间
- Tremor: ~200ms 渲染时间（4x faster）
- Visx: ~100ms 渲染时间（8x faster）
...
### **Week 0: 技术栈升级准备（新增）**
0. ✅ Next.js 15 + React 19 升级
1. ✅ Tailwind v4 升级
2. ✅ shadcn/ui 引入
3. ✅ TanStack Query v5 升级
4. ✅ 图表库升级（Tremor）
5. ✅ 动画库优化（Motion）
