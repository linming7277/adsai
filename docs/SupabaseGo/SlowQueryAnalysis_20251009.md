...
**目标实例**: `adsai`（PostgreSQL 17，私有 IP）
**执行人**: Codex 助手
...
1. **GCP 鉴权与项目配置**
   - `gcloud auth activate-service-account service-account@your-gcp-project-id.iam.gserviceaccount.com --key-file=secrets/gcp_codex_dev.json`
   - `gcloud config set project your-gcp-project-id`

2. **创建审计专用 GCS 存储桶**
   - `gsutil mb -l asia-northeast1 gs://adsai-query-insights-apne1`
...
3. **启用 `pg_stat_statements`**
   - 将 `CREATE EXTENSION IF NOT EXISTS pg_stat_statements;` 上传至 `gs://adsai-query-insights-apne1/ddl/create_pg_stat_statements.sql`
   - 执行 `gcloud sql import sql adsai gs://adsai-query-insights-apne1/ddl/create_pg_stat_statements.sql --database=adsai_db`
   - 校验扩展：`SELECT extname FROM pg_extension` 返回 `pg_stat_statements`

4. **开启慢查询日志阈值**
   - `gcloud sql instances patch adsai --database-flags log_min_duration_statement=100 --quiet`
   - 后续所有超过 100ms 的语句会写入 Cloud Logging（备查）

5. **导出慢查询 & 高频查询数据**
   - `gcloud sql export csv adsai gs://adsai-query-insights-apne1/pg_stat_statements_over100ms_20251009133906.csv --database=adsai_db --query="SELECT … WHERE s.mean_exec_time > 100"`
   - `gcloud sql export csv adsai gs://adsai-query-insights-apne1/pg_stat_statements_postgres_20251009133831.csv --database=adsai_db --query="SELECT … WHERE rolname='postgres'"`
...
- `gs://adsai-query-insights-apne1/pg_stat_statements_over100ms_20251009133906.csv`
- `gs://adsai-query-insights-apne1/pg_stat_statements_postgres_20251009133831.csv`
- `gs://adsai-query-insights-apne1/ddl/create_pg_stat_statements.sql`
