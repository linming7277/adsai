...
\- （可选）Repository Variables：
  ...
  - `ARTIFACT_REPO`: `adsai-services`（Artifact Registry 代码库名）
...
\- 镜像仓库：Google Cloud Artifact Registry，仓库名 `adsai-services`，镜像前缀：`<REGION>-docker.pkg.dev/<PROJECT_ID>/adsai-services/<service>`
\- 分阶段 Job：
...
- 环境映射：`main` → preview（`adsai-api-preview`/`adsai-gw-preview`），`production`/Tag → prod（`adsai-api-prod`/`adsai-gw`）
- 动作：
