...
> 最后更新：2025-10-09  
> 适用范围：AutoAds Monorepo（前端 + 所有 Go 服务 + 运维脚本）
...
|--------|-------------|------------|------------|------|
| `ADS_OAUTH_REDIRECT_URLS` | `ADS_OAUTH_REDIRECT_URLS` | `https://www.urlchecker.dev/dashboard/adscenter/callback` | `https://www.autoads.dev/dashboard/adscenter/callback` | 多个 URL 可用逗号或换行分隔 |
| `ADS_OAUTH_REDIRECT_URL` | `ADS_OAUTH_REDIRECT_URL` | （可选）`https://www.urlchecker.dev/dashboard/adscenter/callback` | （可选）`https://www.autoads.dev/dashboard/adscenter/callback` | 兼容旧字段，推荐配置 `ADS_OAUTH_REDIRECT_URLS` |
| `AUTH_URL` | `AUTH_URL` | `https://www.urlchecker.dev` | `https://www.autoads.dev` | Supabase / NextAuth 登录回调基础地址 |
| `DATABASE_URL` | `DATABASE_URL` | `postgresql://postgres:***@10.6.0.2:5432/autoads_dev` | `postgresql://postgres:***@10.6.0.2:5432/autoads_db` | 内部私网 Postgres 连接串 |
| `DOMAIN` | `DOMAIN` | `urlchecker.dev` | `autoads.dev` | 客户端展示用主域名 |
...
| `NEXTAUTH_URL` | `NEXTAUTH_URL` | `https://www.urlchecker.dev` | `https://www.autoads.dev` | NextAuth callback 绝对 URL |
| `NEXT_PUBLIC_API_BASE_URL` | `NEXT_PUBLIC_API_BASE_URL` | `https://autoads-gw-preview-885pd7lz.an.gateway.dev/api/v1` | `https://autoads-gw-885pd7lz.an.gateway.dev/api/v1` | Cloud Build `substitutions` 切换 |
| `NEXT_PUBLIC_DOMAIN` | `NEXT_PUBLIC_DOMAIN` | `urlchecker.dev` | `autoads.dev` | 旧脚本/容器仍依赖，可与 `DOMAIN` 搭配 |
| `NEXT_PUBLIC_SITE_URL` | `NEXT_PUBLIC_SITE_URL` | `https://www.urlchecker.dev` | `https://www.autoads.dev` | 构建阶段通过 build arg 注入 |
...
| `PROXY_URLS` | 同名 | `https://proxy-preview.autoads.dev` | `https://proxy-prod.autoads.dev` | JSON/逗号分隔代理列表 |
...
| `SUPER_ADMIN_EMAIL` | 同名 | `admin-preview@autoads.dev` | `admin@autoads.dev` | 控制台最高权限账号（可选） |
...
- `scripts/env/export-secrets.sh <PROJECT_ID> <OUTPUT_FILE>`  
  ...
  - 示例：`scripts/env/export-secrets.sh gen-lang-client-0944935873 .env.preview.generated`

建议在 CI 中新增“配置审计”步骤：

```yaml
- name: Audit secrets
  run: python scripts/env/audit_secrets.py --project gen-lang-client-0944935873 --warn-extra
```
...
1. 使用 `codex-dev@gen-lang-client-0944935873.iam.gserviceaccount.com` 服务账号管理 Secret。  
2. 新建/更新：
   ```bash
...
```bash
export PROJECT_ID=gen-lang-client-0944935873
VALUE=$(gcloud secrets versions access latest --secret NEXT_PUBLIC_API_BASE_URL --project "$PROJECT_ID")
docker build --build-arg NEXT_PUBLIC_API_BASE_URL="$VALUE" -t autoads/frontend -f apps/frontend/Dockerfile .
```
...
1. 首次启动时：
   ```bash
   scripts/env/export-secrets.sh gen-lang-client-0944935873 .env.generated
   cp .env.generated .env.local
   ```
2. 不要在仓库提交 `.env.local`；只将调试需要的差异写入 `.env.local`，其余依赖 Secret 导出的模板。
3. 定期运行 `python scripts/env/audit_secrets.py --project gen-lang-client-0944935873 --warn-extra` 确认 secret 未缺失。
