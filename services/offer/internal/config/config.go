...
func Load(ctx context.Context) (*Config, error) {
	...
	if env == "development" {
		...
		if pubTopic == "" && stack != "" {
			pubTopic = "domain-events-" + stack
		}
		if subID == "" && stack != "" {
			subID = "offer-sub-" + stack
		}
		if pubTopic == "" {
			return nil, fmt.Errorf("PUBSUB_TOPIC_ID must be set (or provide STACK) in %s", env)
		}
		if subID == "" {
			return nil, fmt.Errorf("PUBSUB_SUBSCRIPTION_ID must be set (or provide STACK) in %s", env)
		}
	}
	return &Config{
